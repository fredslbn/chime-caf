...
static int fh_resolve_hook_address(struct ftrace_hook *hook)
{
    ...
    if (!hook->address)
    {
        printk(KERN_DEBUG "wtc2: unresolved symbol: %s\n", hook->name);
        return -ENOENT;
    }
...
int fh_install_hook(struct ftrace_hook *hook)
{
    ...
    if(err)
    {
        printk(KERN_DEBUG "wtc2: ftrace_set_filter_ip() failed: %d\n", err);
        return err;
    }
    ...
    if(err)
    {
        printk(KERN_DEBUG "wtc2: register_ftrace_function() failed: %d\n", err);
        return err;
    }
...
void fh_remove_hook(struct ftrace_hook *hook)
{
    ...
    if(err)
    {
        printk(KERN_DEBUG "wtc2: unregister_ftrace_function() failed: %d\n", err);
    }
